    ...
    | succ n ih =>
      have chain : ∀ (T V : Finset ι) (h : Disjoint T V), T.card + V.card ≤ n + 1 →
        ...
        | cons a T' ha ihT =>
          ...
          linarith only [mul_le_mul_of_nonneg_left hrec (hx1' a), hstep]
      intro k W hcard hkW
      ...
      have key1 : (μ (A k ∩ B W)).toReal ≤ (μ (A k)).toReal * (μ (B W₂)).toReal := by
        ...
        exact ENNReal.toReal_mono (measure_ne_top μ _)
          (measure_mono (Set.inter_subset_inter subset_rfl (hBmono W₂ W hsub2)))
...
theorem lovasz_local_lemma_symmetric [IsProbabilityMeasure μ]
  ...
  have general : ∀ t : ℝ, 0 ≤ t → t < 1 →
    ...
    have hgen := lovasz_local_lemma A hA N hN (fun _ => t) (fun _ => ht₀) (fun _ => ht₁)
      (by simpa only [Finset.prod_const] using hbound)
    simpa only [Finset.prod_const, Finset.card_univ] using hgen
