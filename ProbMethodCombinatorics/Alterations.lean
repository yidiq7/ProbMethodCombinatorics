...
theorem exists_isDominating_card_le_of_mem_Icc (G : SimpleGraph V) [DecidableRel G.Adj]
  ...
  -- Keep each vertex independently with probability `p`, then repair by adding every vertex
  -- left undominated.  The average is taken over all `t : Finset V` weighted by
  -- `p ^ |t| * (1 - p) ^ |tᶜ|`.
  set q : ℝ := 1 - p with hqdef
  have hq0 : 0 ≤ q := by rw [hqdef]; linarith
  have hq1 : q ≤ 1 := by rw [hqdef]; linarith
  have hpq : p + q = 1 := by rw [hqdef]; ring
  have hw0 : ∀ t : Finset V, (0 : ℝ) ≤ p ^ t.card * q ^ (tᶜ).card :=
    fun t => mul_nonneg (pow_nonneg hp0 _) (pow_nonneg hq0 _)
  -- The subsets disjoint from a fixed `S` carry total weight `q ^ |S|`; this is the binomial
  -- theorem in the form `Fintype.prod_add`.
  have hkey : ∀ S : Finset V,
      ∑ t : Finset V, (p ^ t.card * q ^ (tᶜ).card) * (if Disjoint t S then (1 : ℝ) else 0)
        = q ^ S.card := by
    intro S
    have h := Fintype.prod_add (fun a : V => if a ∈ S then (0 : ℝ) else p) (fun _ : V => q)
    have hL : (∏ a : V, ((if a ∈ S then (0 : ℝ) else p) + q)) = q ^ S.card := by
      have hcongr : ∀ a : V, ((if a ∈ S then (0 : ℝ) else p) + q) = if a ∈ S then q else 1 := by
        intro a
        by_cases ha : a ∈ S <;> simp [ha, hpq]
      rw [Finset.prod_congr rfl fun a _ => hcongr a, Fintype.prod_ite_mem, Finset.prod_const]
    have hR : ∀ t : Finset V, (∏ a ∈ t, (if a ∈ S then (0 : ℝ) else p))
        = (if Disjoint t S then (1 : ℝ) else 0) * p ^ t.card := by
      intro t
      by_cases hd : Disjoint t S
      · rw [if_pos hd, one_mul]
        rw [Finset.prod_congr rfl fun a ha => if_neg (Finset.disjoint_left.mp hd ha),
          Finset.prod_const]
      · rw [if_neg hd, zero_mul]
        obtain ⟨a, hat, haS⟩ := Finset.not_disjoint_iff.mp hd
        exact Finset.prod_eq_zero hat (if_pos haS)
    rw [hL] at h
    rw [h]
    refine Finset.sum_congr rfl fun t _ => ?_
    rw [hR t, Finset.prod_const]
    ring
  have htot : ∑ t : Finset V, p ^ t.card * q ^ (tᶜ).card = 1 := by
    simpa using hkey ∅
  -- A fixed vertex is kept with total weight `p`.
  have hmem : ∀ v : V,
      ∑ t : Finset V, (p ^ t.card * q ^ (tᶜ).card) * (if v ∈ t then (1 : ℝ) else 0) = p := by
    intro v
    have hsplit : ∀ t : Finset V,
        (p ^ t.card * q ^ (tᶜ).card) * (if v ∈ t then (1 : ℝ) else 0)
          = p ^ t.card * q ^ (tᶜ).card
            - (p ^ t.card * q ^ (tᶜ).card) *
              (if Disjoint t ({v} : Finset V) then (1 : ℝ) else 0) := by
      intro t
      by_cases hv : v ∈ t <;> simp [hv, Finset.disjoint_singleton_right]
    rw [Finset.sum_congr rfl fun t _ => hsplit t, Finset.sum_sub_distrib, htot, hkey {v}]
    simp [hqdef]
  -- A fixed vertex is left undominated only when its closed neighbourhood is missed, which has
  -- total weight at most `q ^ (δ + 1)`.
  have hclosed : ∀ v : V,
      ∑ t : Finset V, (p ^ t.card * q ^ (tᶜ).card) *
          (if ¬ (v ∈ t ∨ ∃ u ∈ t, G.Adj u v) then (1 : ℝ) else 0) ≤ q ^ (δ + 1) := by
    intro v
    have hiff : ∀ t : Finset V,
        (¬ (v ∈ t ∨ ∃ u ∈ t, G.Adj u v)) ↔ Disjoint t (insert v (G.neighborFinset v)) := by
      intro t
      ...
      · rintro h a ha hat
        ...
        · exact h (Or.inl hat)
        · exact h (Or.inr ⟨a, hat, ((SimpleGraph.mem_neighborFinset _ _ _).mp ha').symm⟩)
      · rintro h (hc | ⟨u, hu, huv⟩)
        · exact h (Finset.mem_insert_self _ _) hc
        · exact h (Finset.mem_insert_of_mem
            ((SimpleGraph.mem_neighborFinset _ _ _).mpr huv.symm)) hu
    rw [Finset.sum_congr rfl fun t _ => by rw [if_congr (hiff t) rfl rfl], hkey _]
    have hcard : (insert v (G.neighborFinset v)).card = G.degree v + 1 := by
      rw [Finset.card_insert_of_notMem (by simp)]
      rfl
    rw [hcard]
    exact pow_le_pow_of_le_one hq0 hq1 (by have := hdeg v; omega)
  -- The repaired set: the random set together with the vertices it failed to dominate.
  obtain ⟨W, hW⟩ : ∃ W : Finset V → Finset V,
      ∀ t : Finset V, W t = t ∪ univ.filter (fun v => ¬ (v ∈ t ∨ ∃ u ∈ t, G.Adj u v)) :=
    ⟨_, fun _ => rfl⟩
  have hdom : ∀ t : Finset V, IsDominating G (W t) := by
    intro t v
    rw [hW]
    by_cases h : v ∈ t ∨ ∃ u ∈ t, G.Adj u v
    · rcases h with h | ⟨u, hu, huv⟩
      · exact Or.inl (Finset.mem_union_left _ h)
      · exact Or.inr ⟨u, Finset.mem_union_left _ hu, huv⟩
    · exact Or.inl (Finset.mem_union_right _ (Finset.mem_filter.mpr ⟨Finset.mem_univ _, h⟩))
  have hWcard : ∀ t : Finset V, ((W t).card : ℝ)
      ≤ ∑ v : V, ((if v ∈ t then (1 : ℝ) else 0)
          + (if ¬ (v ∈ t ∨ ∃ u ∈ t, G.Adj u v) then (1 : ℝ) else 0)) := by
    intro t
    rw [Finset.sum_add_distrib, Finset.sum_boole, Finset.sum_boole, hW]
    have h1 : (univ.filter (fun v => v ∈ t)) = t := by simp
    rw [h1]
    exact_mod_cast Finset.card_union_le t
      (univ.filter (fun v => ¬ (v ∈ t ∨ ∃ u ∈ t, G.Adj u v)))
  have havg : ∑ t : Finset V, (p ^ t.card * q ^ (tᶜ).card) * ((W t).card : ℝ)
      ≤ p * Fintype.card V + q ^ (δ + 1) * Fintype.card V := by
    have step1 : ∑ t : Finset V, (p ^ t.card * q ^ (tᶜ).card) * ((W t).card : ℝ)
        ≤ ∑ v : V, ((∑ t : Finset V, (p ^ t.card * q ^ (tᶜ).card) *
              (if v ∈ t then (1 : ℝ) else 0))
            + ∑ t : Finset V, (p ^ t.card * q ^ (tᶜ).card) *
                (if ¬ (v ∈ t ∨ ∃ u ∈ t, G.Adj u v) then (1 : ℝ) else 0)) := by
      have hstep := Finset.sum_le_sum (fun t (_ : t ∈ (Finset.univ : Finset (Finset V))) =>
        mul_le_mul_of_nonneg_left (hWcard t) (hw0 t))
    ...
    have step2 : ∀ v : V,
        ((∑ t : Finset V, (p ^ t.card * q ^ (tᶜ).card) * (if v ∈ t then (1 : ℝ) else 0))
            + ∑ t : Finset V, (p ^ t.card * q ^ (tᶜ).card) *
                (if ¬ (v ∈ t ∨ ∃ u ∈ t, G.Adj u v) then (1 : ℝ) else 0)) ≤ p + q ^ (δ + 1) := by
      intro v
      rw [hmem v]
      linarith [hclosed v]
    refine (Finset.sum_le_sum fun v (_ : v ∈ (Finset.univ : Finset V)) => step2 v).trans ?_
    rw [Finset.sum_const, Finset.card_univ, nsmul_eq_mul]
    ring_nf
    exact le_of_eq (by ring)
  obtain ⟨t₀, -, ht₀⟩ := Finset.exists_min_image (Finset.univ : Finset (Finset V))
    (fun t => (W t).card) ⟨∅, Finset.mem_univ _⟩
  refine ⟨W t₀, hdom t₀, ?_⟩
  have h1 : ∑ t : Finset V, (p ^ t.card * q ^ (tᶜ).card) * ((W t₀).card : ℝ)
      ≤ ∑ t : Finset V, (p ^ t.card * q ^ (tᶜ).card) * ((W t).card : ℝ) :=
    Finset.sum_le_sum fun t _ => mul_le_mul_of_nonneg_left
      (by exact_mod_cast ht₀ t (Finset.mem_univ t)) (hw0 t)
  rw [← Finset.sum_mul, htot, one_mul] at h1
  exact h1.trans havg
...
theorem exists_isDominating_card_le (G : SimpleGraph V) [DecidableRel G.Adj]
  ...
  -- The averaging step, for an arbitrary `p ∈ [0, 1]`: keep each vertex independently with
  -- probability `p`, then repair by adding every vertex left undominated.  The average is taken
  -- over all `t : Finset V` weighted by `p ^ |t| * (1 - p) ^ |tᶜ|`.
  have key : ∀ p : ℝ, 0 ≤ p → p ≤ 1 → ∃ U : Finset V, IsDominating G U ∧
      (U.card : ℝ) ≤ p * Fintype.card V + (1 - p) ^ (δ + 1) * Fintype.card V := by
    intro p hp0 hp1
    set q : ℝ := 1 - p with hqdef
    have hq0 : 0 ≤ q := by rw [hqdef]; linarith
    ...
    rw [← Finset.sum_mul, htot, one_mul] at h1
    exact h1.trans havg
  -- The optimisation step, at `p = log (δ + 1) / (δ + 1)`.
  have hn : (0 : ℝ) < (δ : ℝ) + 1 := by positivity
  have hp0 : 0 ≤ Real.log ((δ : ℝ) + 1) / ((δ : ℝ) + 1) :=
    div_nonneg (Real.log_nonneg (by linarith [Nat.cast_nonneg (α := ℝ) δ])) hn.le
  have hp1 : Real.log ((δ : ℝ) + 1) / ((δ : ℝ) + 1) ≤ 1 := by
    rw [div_le_one hn]
    linarith [Real.log_le_sub_one_of_pos hn]
  obtain ⟨U, hU, hcard⟩ := key _ hp0 hp1
  refine ⟨U, hU, hcard.trans ?_⟩
  have hcardV : (0 : ℝ) ≤ (Fintype.card V : ℝ) := Nat.cast_nonneg _
  have hopt := log_div_add_one_add_pow_le δ hδ
  nlinarith [hopt, hcardV]
